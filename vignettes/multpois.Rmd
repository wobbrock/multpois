---
title: "Analyzing nominal responses via the multinomial-Poisson trick"
author: "Jacob O. Wobbrock"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Analyzing nominal responses via the multinomial-Poisson trick}
...
certain experiments or surveys, the dependent variable can be one of *N* categories. For example,
we might ask people what their favorite ice cream flavor is: vanilla, chocolate, strawberry, or
other. This four-category response would be a polytomous dependent variable. Perhaps
we wish to ask adults and children about their favorite ice cream to see if there is a difference by
age group. We would then have a two-level between-subjects factor. If we ask each respondent only once, 
this data set would represent a one-way between-subjects design. But perhaps we ask each participant 
once *each season*&mdash;in fall, winter, spring, and summer&mdash;to see if their responses change. Now 
we would have a four-level within-subjects factor, i.e., repeated measures. 

The `multpois` package helps us analyze this type of data, where the dependent variable is nominal. It does 
so by modeling nominal responses as counts of category choices and uses (mixed) Poisson regression 
to analyze these counts (Baker 1994, Chen &amp; Kuo 2001). This technique is known as the  
multinomial-Poisson transformation (Guimaraes 2004) or trick (Lee et al. 2017).
...
* If the response is polytomous, and one or more factors is within-subjects, there is no easy
  option similar to the three above. The `multinom` function in `nnet` cannot take random factors
  to handle repeated measures, and the `glmer` function in `lme4` does not offer a 
  `family=multinomial` option. It is this case in particular that this package was created to
  address, although it can address the above three scenarios, also.
...
```{r fig.cap="**Figure 1.** Proportions of `no` (pink) and `yes` (green) responses in four conditions: `{a, c}`, `{a, d}`, `{b, c}`, and `{b, d}`.", fig.height=4.5, fig.width=4}
...
and the results from the *post hoc* pairwise comparisons are quite similar.
...
```{r fig.cap="**Figure 2.** Proportions of `maybe` (yellow), `no` (pink), and `yes` (green) responses in four conditions: `{a, c}`, `{a, d}`, `{b, c}`, and `{b, d}`.", fig.height=4.5, fig.width=4}
...
The results from the *post hoc* pairwise comparisons are fairly similar.
...
```{r fig.cap="**Figure 3.** Proportions of `no` (pink) and `yes` (green) responses in four conditions: `{a, c}`, `{a, d}`, `{b, c}`, and `{b, d}`.", fig.height=4.5, fig.width=4}
...
`glmer` from the `lme4` package provides this for us:
...
This fourth example is the reason that the `multpois` package was created. Unlike the three
examples above, there are not straightforward options for analyzing nominal responses with repeated 
measures and obtaining ANOVA-style results. Some functions do offer mixed-effects multinomial regression modeling, such as `mblogit` in the `mclogit` package, but they do not enable ANOVA-style output. Other advanced methods exist, such as Markov chain Monte Carlo (MCMC) methods in the `MCMCglmm` library, which does have a
`family=multinomial` option, but these methods are complex and deviate from the approaches illustrated 
...
```{r fig.cap="**Figure 4.** Proportions of `maybe` (yellow), `no` (pink), and `yes` (green) responses in four conditions: `{a, c}`, `{a, d}`, `{b, c}`, and `{b, d}`.", fig.height=4.5, fig.width=4}
xt = xtabs( ~ X1 + X2 + Y, data=ws3)
mosaicplot(xt, main="Y by X1, X2", las=1, col=c("lightyellow","pink","lightgreen"))
```

Because `multinom` from the `nnet` package cannot handle random factors, it cannot model repeated measures. 
...
a polytomous response, one between-subjects factor, and one within-subjects factors. This mixed factorial
design is also known as a split-plot design. 

This fictional data is based on the scenario at the beginning of this vignette. Eighty respondents, half
...
with four levels `{fall, winter, spring, summer}`. The polytomous response, `Pref`, has four categories: 
`{vanilla, chocolate, strawberry, other}`. The `PId` factor is repeated across rows, indicating
respondents were queried four times each.
...
```{r fig.cap="**Figure 5.** Proportions of `chocolate` (brown), `other` (gray), `strawberry` (pink), and `vanilla` (beige) responses in eight conditions: `{adult, fall}`, `{adult, spring}`, `{adult, sumer}`, `{adult, winter}`, `{child, fall}`, `{child, spring}`, `{child, summer}`, and `{child, winter}`.", fig.height=6, fig.width=7}
xt = xtabs( ~ Age + Season + Pref, data=icecream)
mosaicplot(xt, main="Pref by Age, Season", las=2, col=c("tan","gray","pink","beige"))
```
...
We have a main effect of `Season` and an `Age&times;Season` interaction but no main effect of `Age`. We 
can explore this further by graphically depicting response proportions in each age group:

```{r fig.cap="**Figure 6.** Proportions of `chocolate` (brown), `other` (gray), `strawberry` (pink), and `vanilla` (beige) responses for adults and children. Although the proportions differ, they are similar, illustrating the non-significant effect of `Age`.", fig.height=6, fig.width=7}
xt = xtabs( ~ Age + Pref, data=icecream)
mosaicplot(xt, main="Pref by Age", las=1, col=c("tan","gray","pink","beige"))
```

For completeness, let's also graphically depict the main effect of `Season`:

```{r fig.cap="**Figure 7.** Proportions of `chocolate` (brown), `other` (gray), `strawberry` (pink), and `vanilla` (beige) responses for each of four seasons. The differences in response proportions across seasons clearly emerge, illustrating the significant effect of `Season`. Fall and winter are clearly the season for chocolate, spring for strawberry, and vanilla is always in style!", fig.height=6, fig.width=7}
xt = xtabs( ~ Season + Pref, data=icecream)
mosaicplot(xt, main="Pref by Season", las=1, col=c("tan","gray","pink","beige"))
```
...
*in each season*, not across seasons. In any case, we conduct all pairwise comparisons below simply for 
illustration:
...
If we did wish to compare adults vs. children in each season (fall, winter, spring, summer), we would 
conduct all pairwise comparisons, leaving them *uncorrected*, extract the relevant ones 
(rows 4, 22, 11, 17, respectively), and manually correct their *p*-values, like so:

```{r message=FALSE, warning=FALSE}
glmer.mp.con(m8, pairwise ~ Age*Season, adjust="none")
p.adjust(c(0.005673, 0.107523, 0.042385, 0.007731), method="holm")
```

Thus, after Type I error correction using Holm's sequential Bonferroni procedure 
[(Holm 1979)](http://www.jstor.org/stable/4615733), we see adults vs. children in 
fall and summer are significantly different (*p* < .05). Looking again at Figure 5 visually
